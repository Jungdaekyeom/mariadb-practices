...
select count(*) as '부서갯수' from departments; -- 테이블을 보고 중복이 없어 중복 제거 하지 않고 전체로 계산
select count(distinct dept_no) as '부서갯수' from departments; -- 혹시나 중복이 있을 것을 대비, 각각 계산
select count(distinct dept_name) as '부서갯수' from departments; -- 혹시나 중복이 있을 것을 대비, 각각 계산

-- 문제6.
-- 현재 부서 매니저는 몇 명이나 있나요?(역임 매너저는 제외)
-- count, 현재(9999-01-01) 
-- select * from dept_manager;
select count(distinct emp_no) from dept_manager where to_date = '9999-01-01';
...
select distinct dept_name from departments order by length(dept_name) desc; -- 바이트 수
select distinct dept_name from departments order by char_length(dept_name) desc; -- 글자 수
...
select distinct title from titles order by length(title) desc; -- 바이트 수
select distinct title from titles order by char_length(title) desc; -- 글자 수
...
select count(emp_no) from titles where to_date = '9999-01-01' and title = 'Engineer';
